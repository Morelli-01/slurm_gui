...
class NewJobDialog(QDialog):
    ...
    def _prefetch_data(self):
        ...
        try:
            ...
            self.nodelist_options = [n["NodeName"]
                                     for n in nodes_info if "NodeName" in n]
    ...
    def _setup_essential_tab(self):
        ...
        job_info_layout.addRow(self._create_label(
            "Job Name*:"), self.job_name_edit)

        self.job_comment_edit = QLineEdit()
        self.job_comment_edit.setPlaceholderText(
            "Optional description for the job")
        job_info_layout.addRow(self._create_label(
            "Job Comment:"), self.job_comment_edit)
        ...
        if hasattr(self, 'accounts'):
            self.account_combo.addItems(
                ["None"] + self.accounts if len(self.accounts) == 0 else self.accounts)
    ...
    def _setup_advanced_tab(self):
        ...
        constraints_layout.addRow(self._create_label(
            "Node Constraints:"), self.constraint_combo)
        ...
        constraints_layout.addRow(self._create_label(
            "Nodelist (specific nodes):"), self.nodelist_combo)
        ...
        file_layout.addRow(self._create_label(
        ...
        file_layout.addRow(self._create_label(
            "Error File:"), self.error_file_edit)
        ...
        self.working_dir_edit.setText(self.slurm_connection.remote_home)
    ...
    def _setup_job_dependencies_tab(self):
        ...
        # --- Place refresh and add buttons in the same line ---
        jobs_btns_layout = QHBoxLayout()
        ...
        jobs_btns_layout.addWidget(self.refresh_jobs_button)
        jobs_btns_layout.addStretch()
        ...
        jobs_btns_layout.addWidget(self.add_job_button)
        running_jobs_layout.addLayout(jobs_btns_layout)
        # ---
    ...
    def _browse_venv(self):
        ...
        dialog = RemoteDirectoryDialog(
            self.slurm_connection, initial_path=current_path, parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            selected_directory = dialog.get_selected_directory()
            if selected_directory:
                ...
                if not self.slurm_connection.remote_path_exists(os.path.join(selected_directory, "bin", "activate")):
                    show_warning_toast(
                        self, "Warning", "The selected directory doesn't appear to be a Python virtual environment (no bin/activate found).")
    ...
    def get_job_details(self):
        ...
        if hasattr(self, 'dep_type_combo'):
            dep_type = self.dep_type_combo.currentText().split(
                " - ")[0].strip()
    ...
    def preview_job_script(self):
        ...
        if not self.slurm_connection:
            self.preview_text.setText(
                "Error: No SLURM connection available for preview generation.")
            return
