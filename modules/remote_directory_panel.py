...
class RemoteDirectoryDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.up_button.setIcon(
            QIcon(os.path.join(script_dir, "src_static", "prev_folder.svg")))
        self.up_button.setIconSize(
            QSize(ICON_SIZE, ICON_SIZE))  # Set larger icon size
        self.up_button.setToolTip("Go Up")
        self.up_button.clicked.connect(self._go_up_directory)
        self.up_button.setFixedSize(
            QSize(BUTTON_SIZE, BUTTON_SIZE))  # Increase button size
        nav_layout.addWidget(self.up_button)

        # Home button
        self.home_button = QToolButton()
        self.home_button.setIcon(
            QIcon(os.path.join(script_dir, "src_static", "home.svg")))
        self.home_button.setIconSize(
            QSize(ICON_SIZE, ICON_SIZE))  # Set larger icon size
    ...
    def _load_current_path(self):
        ...
        if self.current_remote_path in self.directory_cache:
            ...
            self._process_directory_results(
                dirs, self.current_remote_path, True)
            return

        # Create and start worker
        self.worker = DirectoryWorker(
            self.slurm_connection, self.current_remote_path)
    ...
    def _process_directory_results(self, directories, path, success):
        ...
        if not directories:
            ...
            for d in directories:
                ...
                item.setIcon(
                    QIcon(os.path.join(script_dir, "src_static", "folder.svg")))
                self.dir_list_model.appendRow(item)
    ...
    def _filter_directories(self, text):
        ...
        if text:
            self.status_label.setText(
                f"Showing {visible_count} directories matching '{text}'")
        else:
            self.status_label.setText(
                f"Showing all {len(self.all_directories)} directories")
    ...
    def _on_item_clicked(self, index):
        ...
        if item_text == "..":
            ...
            self.selected_path = os.path.join(
                self.current_remote_path, item_text)
