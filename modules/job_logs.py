...
class ContinuousLogFetcherThread(QThread):
    ...
    def __init__(self, slurm_connection: SlurmConnection, project: Project, job: Job):
        super().__init__()
    ...
    def run(self):
        ...
        while not self._stop_requested:
            try:
                ...
                stdout, stderr = self.slurm_connection.get_job_logs(self.job, preserve_progress_bars=True)
...
class JobLogsDialog(QDialog):
    def __init__(self, project: Project, job: Job, project_store, parent=None):
        super().__init__(parent)
        self.project = project
        self.job = job
        self.project_store = project_store
        self.slurm_connection = getattr(self.project_store, 'slurm', None)
        self.setWindowTitle(f"Job Details - {job.name} [{job.id}]")
    ...
    def _setup_ui(self):
        ...
        title_label = QLabel(f"Job Details: {self.job.name} [{self.job.id}]")
    ...
    def _load_job_data(self):
        ...
        if not self.job:
            self._show_error("Job object not available")
            return
    ...
    def _start_continuous_fetching(self):
        ...
        self.log_fetcher_thread = ContinuousLogFetcherThread(
            self.slurm_connection, self.project, self.job
        )
