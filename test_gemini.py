...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QSpinBox, QFileDialog, QProgressBar, QMessageBox, QGridLayout
)
...
class SlurmJobManagerApp(QMainWindow):
    def __init__(self):
        ...
        # self.connection_status.setIcon(
        #     QIcon("/home/nicola/Desktop/slurm_gui/src_static/cloud_off_24dp_EA3323_FILL0_wght400_GRAD0_opsz24.png"))
    ...
    def set_connection_status(self, connected: bool, connecting=False):
        if connecting:
            ...
            loading_movie.setScaledSize(QSize(48, 48))
            loading_movie.start()
            self.connection_status.setMovie(loading_movie)
            self.connection_status.setToolTip("Connecting...")
            # loading_movie.setScaledSize(QSize(24, 24))
            # self.connection_status.setText("Connecting...")
            # self.connection_status.setIcon(QIcon(loading_movie.currentPixmap()))
            ...
            # Continuously update the icon with each new frame of the GIF
            # loading_movie.frameChanged.connect(
            #     lambda _: self.connection_status.setIcon(QIcon(loading_movie.currentPixmap())))
            return

        if connected:
            self.connection_status.setToolTip("Connected")
            self.connection_status.setPixmap(
                QIcon("/home/nicola/Desktop/slurm_gui/src_static/good_connection.png").pixmap(QSize(48, 48)))
            self.connection_status.setStyleSheet("""
                QPushButton#statusButton {
            ...
            self.connection_status.setPixmap(
                QIcon("/home/nicola/Desktop/slurm_gui/src_static/bad_connection.png").pixmap(QSize(48, 48)))
            self.connection_status.setStyleSheet("""
                QPushButton#statusButton {
