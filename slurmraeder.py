...
def _node_from_sinfo(x):
    n = Node(_node_preproc(x['NODELIST']), x.n_gpu, x.m_gpu, x.reserved,
             None, x.CPUS, x.MEMORY)  # , feature=x.AVAIL_FEATURES)
...
def _read_nodes(reservations=None, pending_res=None, return_raw=False):
    ...
    sinfo_df.loc[:, 'n_gpu'] = sinfo_df['GRES'].apply(lambda x: 0 if x.split(
        '(')[0].split(':')[0] in ('(null)', '') else int(x.split('(')[0].split(':')[-1]))
...
def _read_limits():
    ...
    """
    lims = pd.read_fwf(StringIO(os.popen(
        r'sacctmgr show qos format=\'name%15,maxtresperuser%35,grptres%25,maxjobsperuser\' 2>/dev/null').read())).loc[1:]
    lims['puG'] = lims['MaxTRESPU'].apply(lambda x: int(
        x.split('gres/gpu=')[1].split(',')[0]) if isinstance(x, str) and 'gres/gpu' in x else float('nan'))
    lims['grpG'] = lims['GrpTRES'].apply(lambda x: int(x.split('gres/gpu=')[1].split(',')[0])
                                         if isinstance(x, str) and 'gres/gpu' in x else float('nan'))
    lims['puM'] = lims['MaxTRESPU'].apply(lambda x: parse_mem(x.split('mem=')[1].split(',')[
                                          0]) if isinstance(x, str) and 'mem=' in x else float('nan'))
    lims['grpM'] = lims['GrpTRES'].apply(lambda x: parse_mem(x.split('mem=')[1].split(',')[
                                         0]) if isinstance(x, str) and 'mem=' in x else float('nan'))
    lims['puC'] = lims['MaxTRESPU'].apply(lambda x: int(x.split('cpu=')[1].split(
        ',')[0]) if isinstance(x, str) and 'cpu=' in x else float('nan'))
    lims['grpC'] = lims['GrpTRES'].apply(lambda x: int(x.split('cpu=')[1].split(
        ',')[0]) if isinstance(x, str) and 'cpu=' in x else float('nan'))
...
def _cpus_per_joblet(line):
    try:
        if 'node=1' in line['TRES_ALLOC']:
            ...
            nodes = line['NODES']
            tot_gpu = int(str(line['TRES_ALLOC']).split('gpu=')[-1].split(',')
                          [0]) if 'gpu=' in str(line['TRES_ALLOC']) else 0
...
def read_jobs():
    ...
    squeue_df['gpus_per_node'] = squeue_df['TRES_PER_NODE'].apply(
        lambda x: try_parse_gpu(x) if isinstance(x, str) else x)
    squeue_df['gpus_per_job'] = squeue_df['TRES_PER_JOB'].apply(lambda x: try_parse_gpu(x) if isinstance(x, str) else x)
    squeue_df['gpus_per_task'] = squeue_df['TRES_PER_TASK'].apply(
        lambda x: try_parse_gpu(x) if isinstance(x, str) else x)
